...
from moss_crewai import uninstall
...
def _run(monkeypatch, args):
    monkeypatch.setattr(sys, "argv", ["moss_crewai.uninstall", *args])
...
def test_manifest_stays_valid_only_moss_removed(tmp_path, monkeypatch):
    ...
    if tomllib is not None:
        ...
        assert any("crewai" in d for d in deps)
...
def test_no_network_imports():
    ...
    source = Path(uninstall.__file__).read_text()
