"""MOSS Uninstall Helper for moss-crewai.

Usage:
    python -m moss_crewai.uninstall [--dry-run]
...
DIST_NAME = "moss-crewai"             # display name
...
def remove_env_vars(dry_run: bool) -> list[str]:
    ...
    for name in ENV_FILES:
        ...
        if matches:
            print(f"[DRY-RUN] Would remove from {path}: {len(matches)} MOSS_* vars"
                  if dry_run else f"Removed {len(matches)} MOSS_* vars from {path}")
            if not dry_run:
...
def remove_dependency(dry_run: bool) -> bool:
    ...
    for manifest in ("requirements.txt", "pyproject.toml"):
        ...
        if pat.search(content):
            print(f"[DRY-RUN] Would remove {DEP_PATTERN} from {manifest}"
                  if dry_run else f"Removed {DEP_PATTERN} from {manifest}")
            if not dry_run:
